...
class DsChoiceChip extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final Color containerBg = theme.brightness == Brightness.light
        ? cs.surfaceVariant // light gray like #EDEDED
    ...
    final Color tickIcon = selected ? (color == DsChipColor.neutral ? cs.onSurface : cs.onPrimary) : cs.onSurface.withOpacity(0.6);
    ...
    final Widget tick = Container(
      ...
      decoration: BoxDecoration(
        color: tickBg,
        border: tickBorder == BorderSide.none ? null : Border.fromBorderSide(tickBorder),
    ...
    final decorated = DecoratedBox(
      decoration: BoxDecoration(
        ...
        border: Border(
          ...
          left: border,
          right: border,
          bottom: BorderSide(color: border.color, width: (selected ? 2 : 1).toDouble()),
        ),
      ),
      child: content,
    );
